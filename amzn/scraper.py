...
class AmazonScraper:
	...
	def __init__(self, *args, **kwargs):
		...
		self.image_dir = kwargs['image_dir'] if 'image_dir' in kwargs else './imgs'
		self.demo_mode = kwargs['demo_mode'] if 'demo_mode' in kwargs else False
		self.base_url = "https://www.amazon.com/s/ref=nb_sb_noss?url=search-alias%3Daps&field-keywords="
		self.proxy_list = args
		self.error = ErrorLog('./error.log')
		self.proxy = Proxy()
		if len(self.proxy_list) > 0:
			self.proxies = { 'http': self.proxy_list[0] }
		else:
	...
	def save_image(self, name, image_link):
		file_name = "{}/{}-{}.jpg".format(self.image_dir, re.sub('[^A-z\s]*', '', name).replace(' ', '_'), str(datetime.datetime.now().timestamp()).split('.')[0])
		if self.verbose:
	...
	def get_page(self, page_link='', attempts=0):
		if page_link == '':
			page_link = self.base_url
		headers = { 'User-Agent': generate_user_agent(device_type='desktop', os=('mac', 'linux')) }
		if self.verbose:
			print("Scraping {}...".format(page_link))
		try:
			# after a few failed attempts, fall back to trusted IP
			if attempts < 2:
